# -*- coding: utf-8 -*-
# [K-EDGE V9.5.2e QUEUE SCANNER] One-way 청산 패치 - 복리/OPEN복구/정지복구/FX파일캐시/안전장치 적용
"""
...
# 너무 많은 검사 방지
MAX_SPOT_ITEMS = int(os.getenv("MEXC_MAX_SPOT_ITEMS", "45"))  # V9.3.2 MEXC 전용 저속모드
...
# 국내 거래소별 최대 후보 수
MAX_UPBIT_ITEMS = 0
MAX_BITHUMB_ITEMS = int(os.getenv("MEXC_MAX_BITHUMB_ITEMS", "45"))  # V9.3.2 MEXC 전용 저속모드
MAX_GOPAX_ITEMS = 40
MAX_COINONE_ITEMS = 0
MAX_KORBIT_ITEMS = 40
...
# - 주문/큐/텔레그램/청산 로직과 분리
# - 실패해도 봇은 계속 진행
# - 실제 DB 컬럼명 기준: domestic_exchange / foreign_exchange / real_edge_percent / executable_krw / price_gap_per
# ============================================================
...
def supabase_live_headers() -> Dict[str, str]:
    return {
...
def push_live_event_candidate(signal: Dict[str, Any]) -> None:
    ...
    중요: 홈페이지 표시용만 담당한다. 주문/큐/텔레그램 로직은 건드리지 않는다.
    """
    try:
...
AUTO_CLOSE_ENABLED = False  # V9.5.2 scanner only: close worker handles exits
...
ENABLE_CALLBACK_POLLER = False  # V9.5.2 scanner only: workers handle control/buttons
...
# ============================================================
# V9.5.2 QUEUE ARCHITECTURE - SCANNER ONLY
# - Scanner files do NOT place real orders.
# - Scanner files only append candidate signals to order_queue.jsonl.
# - kedge_order_worker_v9_5_2.py performs real entry orders.
# ============================================================
ORDER_QUEUE_PATH = os.path.join(BASE_DIR, "order_queue.jsonl")
ORDER_QUEUE_LOCK_PATH = os.path.join(BASE_DIR, "order_queue.lock")
SCAN_QUEUE_DEDUP_SEC = int(os.getenv("SCAN_QUEUE_DEDUP_SEC", "20"))
_SCAN_QUEUE_LAST_AT: Dict[str, float] = {}
_SCAN_QUEUE_LOCK = threading.Lock()


def _queue_json_default(obj: Any) -> Any:
    try:
...
def enqueue_order_candidate(signal: Dict[str, Any]) -> bool:
    """Append one candidate signal to order_queue.jsonl.

    This function is intentionally fast and local-file based.
    The order worker performs member lookup, final recheck, global lock,
    foreign short entry, Bithumb spot buy, ACTIVE save and DM.
    """
    try:
        ...
        with _SCAN_QUEUE_LOCK:
            last = float(_SCAN_QUEUE_LAST_AT.get(signal_id, 0.0))
            if now_ts - last < SCAN_QUEUE_DEDUP_SEC:
                print(f"[QUEUE SKIP] recent duplicate signal_id={signal_id}")
                return False
            _SCAN_QUEUE_LAST_AT[signal_id] = now_ts
        ...
        payload = {
            ...
            "real_edge": signal.get("real_edge"),
            "signal": signal,
        }
        os.makedirs(os.path.dirname(ORDER_QUEUE_PATH), exist_ok=True)
        line = json.dumps(payload, ensure_ascii=False, default=_queue_json_default)
        with open(ORDER_QUEUE_PATH, "a", encoding="utf-8") as f:
            f.write(line + "\n")
        print(
            f"[QUEUE 저장] {signal.get('coin')} {signal.get('domestic')}->{signal.get('foreign')} "
            f"edge={safe_float(signal.get('real_edge')):+.2f}% signal_id={signal_id}"
        )
        return True
    except Exception as e:
...
OPEN_ENTRY_STATUSES = {"OPEN", "VIRTUAL_OPEN", "REAL_OPEN", "ACTIVE", "FUTURES_CLOSED_ONLY", "SPOT_CLOSED_ONLY"}
...
MEXC_REQUEST_INTERVAL_SEC = float(os.getenv("MEXC_REQUEST_INTERVAL_SEC", "0.32"))
MEXC_RATE_LIMIT_COOLDOWN_SEC = float(os.getenv("MEXC_RATE_LIMIT_COOLDOWN_SEC", "6"))
...
MIN_ALLOWED_SLIPPAGE_FOR_REAL_ENTRY_PERCENT = float(os.getenv("MIN_ALLOWED_SLIPPAGE_FOR_REAL_ENTRY_PERCENT", "0.0"))
...
# ============================================================
# V9.5.3 ORDER TRACE HELPERS
# - 신규 실거래 진입부터 주문번호/체결량을 포지션과 CSV에 저장한다.
# - 과거 포지션은 주문번호가 없으므로 잔고 기반 보조 감사만 가능하다.
# ============================================================
...
def _dig_order(order: Any, *keys: str) -> Any:
    cur = order
    for key in keys:
        if cur is None:
            return None
...
def extract_real_order_trace(signal: Dict[str, Any]) -> Dict[str, Any]:
    """signal['real_order_result']에서 국내/해외 주문번호와 체결 정보를 최대한 추출한다.

    Bithumb v2 직접주문은 {'raw': {...}, 'http': 201, 'params': {...}} 형태이고,
    해외 ccxt 주문은 id/status/filled/average/cost/amount 및 info를 포함한다.
    """
    rr = signal.get("real_order_result") or {}
    if not isinstance(rr, dict):
        rr = {}
    domestic_order = rr.get("domestic_order") or {}
    foreign_order = rr.get("foreign_order") or {}

    d_raw = domestic_order.get("raw") if isinstance(domestic_order, dict) else {}
    if not isinstance(d_raw, dict):
        d_raw = {}
    f_info = foreign_order.get("info") if isinstance(foreign_order, dict) else {}
    if not isinstance(f_info, dict):
        f_info = {}

    d_exec_qty = safe_float(_first_non_empty(
        d_raw.get("executed_volume"),
        domestic_order.get("filled") if isinstance(domestic_order, dict) else None,
        signal.get("domestic_filled_qty"),
    ))
    d_exec_funds = safe_float(_first_non_empty(
        d_raw.get("executed_funds"),
        domestic_order.get("cost") if isinstance(domestic_order, dict) else None,
        signal.get("domestic_actual_fill_krw"),
    ))
    d_avg = safe_float(_first_non_empty(
        d_raw.get("price"),
        domestic_order.get("average") if isinstance(domestic_order, dict) else None,
        signal.get("spot_best_ask"),
    ))
    if d_avg <= 0 and d_exec_qty > 0 and d_exec_funds > 0:
        d_avg = d_exec_funds / d_exec_qty

    f_filled = safe_float(_first_non_empty(
        foreign_order.get("filled") if isinstance(foreign_order, dict) else None,
        f_info.get("executedQty"),
        f_info.get("size"),
        signal.get("foreign_filled_qty"),
    ))
    f_avg = safe_float(_first_non_empty(
        foreign_order.get("average") if isinstance(foreign_order, dict) else None,
        f_info.get("avgPrice"),
        f_info.get("fill_price"),
        foreign_order.get("price") if isinstance(foreign_order, dict) else None,
    ))
    f_cost = safe_float(_first_non_empty(
        foreign_order.get("cost") if isinstance(foreign_order, dict) else None,
        f_info.get("cost"),
        signal.get("foreign_actual_fill_krw"),
    ))

    return {
        "domestic_order_id": str(_first_non_empty(
            d_raw.get("uuid"),
            domestic_order.get("id") if isinstance(domestic_order, dict) else None,
            domestic_order.get("orderId") if isinstance(domestic_order, dict) else None,
        ) or ""),
        "domestic_order_status": str(_first_non_empty(
            d_raw.get("state"),
            domestic_order.get("status") if isinstance(domestic_order, dict) else None,
        ) or ""),
        "domestic_filled_qty": d_exec_qty,
        "domestic_executed_funds": d_exec_funds,
        "domestic_avg_price": d_avg,
        "foreign_order_id": str(_first_non_empty(
            foreign_order.get("id") if isinstance(foreign_order, dict) else None,
            f_info.get("orderId"),
            f_info.get("orderID"),
            f_info.get("id"),
        ) or ""),
        "foreign_client_order_id": str(_first_non_empty(
            foreign_order.get("clientOrderId") if isinstance(foreign_order, dict) else None,
            f_info.get("clientOrderId"),
            f_info.get("clientOrderID"),
        ) or ""),
        "foreign_order_status": str(_first_non_empty(
            foreign_order.get("status") if isinstance(foreign_order, dict) else None,
            f_info.get("status"),
            f_info.get("finish_as"),
        ) or ""),
        "foreign_filled_qty": f_filled,
        "foreign_avg_price": f_avg,
        "foreign_cost": f_cost,
        "foreign_order_amount": safe_float(foreign_order.get("amount") if isinstance(foreign_order, dict) else None),
        "foreign_reduce_only": bool(_first_non_empty(
            foreign_order.get("reduceOnly") if isinstance(foreign_order, dict) else None,
            f_info.get("reduceOnly"),
            f_info.get("is_reduce_only"),
            False,
        )),
    }


def enrich_signal_with_order_trace(signal: Dict[str, Any]) -> Dict[str, Any]:
    trace = extract_real_order_trace(signal)
    signal["order_trace"] = trace
...
def paper_record_entry(pos_id: str, user_id: str, signal: Dict[str, Any], amount_krw: int) -> None:
    ...
    signal = enrich_signal_with_order_trace(signal)
    order_trace = signal.get("order_trace") or {}
    ...
    fieldnames = [
        ...
        "domestic_order_id", "domestic_order_status", "domestic_filled_qty", "domestic_avg_price", "domestic_executed_funds",
        "foreign_order_id", "foreign_client_order_id", "foreign_order_status", "foreign_filled_qty", "foreign_avg_price", "foreign_cost", "foreign_order_amount",
    ...
    row = {
        ...
        "foreign_margin_krw": int(calc_required_foreign_margin_krw(foreign_entry_krw)),
        "futures_leverage": safe_float(signal.get("futures_leverage"), PAPER_FUTURES_LEVERAGE),
        "domestic_order_id": order_trace.get("domestic_order_id", ""),
        "domestic_order_status": order_trace.get("domestic_order_status", ""),
        "domestic_filled_qty": order_trace.get("domestic_filled_qty", 0),
        "domestic_avg_price": order_trace.get("domestic_avg_price", 0),
        "domestic_executed_funds": order_trace.get("domestic_executed_funds", 0),
        "foreign_order_id": order_trace.get("foreign_order_id", ""),
        "foreign_client_order_id": order_trace.get("foreign_client_order_id", ""),
        "foreign_order_status": order_trace.get("foreign_order_status", ""),
        "foreign_filled_qty": order_trace.get("foreign_filled_qty", 0),
        "foreign_avg_price": order_trace.get("foreign_avg_price", 0),
        "foreign_cost": order_trace.get("foreign_cost", 0),
        "foreign_order_amount": order_trace.get("foreign_order_amount", 0),
        # V9.5.3: 주문 응답에서 실제 체결값을 우선 저장한다. 없으면 기존 요청금액으로 fallback.
        "domestic_actual_fill_krw": int(safe_float(order_trace.get("domestic_executed_funds"), safe_float(signal.get("domestic_actual_fill_krw"), domestic_entry_krw))),
...
def member_has_active_coin_position(member: Dict[str, Any], signal: Dict[str, Any]) -> bool:
    """해당 회원이 같은 ROUTE의 미종료 포지션을 이미 보유 중인지 확인.

    V9.5.2f:
    - 기존: 같은 코인 전체 차단. 예) SAHARA-GATE 보유 중이면 SAHARA-MEXC도 차단.
    - 변경: 같은 코인 + 같은 국내거래소 + 같은 해외거래소 ROUTE만 차단.
    - 예) SAHARA BITHUMB↔GATE 보유 중이어도 SAHARA BITHUMB↔MEXC / BITGET / BINGX는 진입 가능.
    - 단, 동일 route 재진입은 ACTIVE/FUTURES_CLOSED_ONLY/SPOT_CLOSED_ONLY 동안 계속 차단.
    """
    tg_id = str(get_member_chat_id(member) or "")
    coin = normalize_symbol(signal.get("coin") or "")
    domestic = str(signal.get("domestic") or signal.get("domestic_exchange") or "").upper().strip()
    foreign = str(signal.get("foreign") or signal.get("foreign_exchange") or "").upper().strip()
    if not tg_id or not coin or not domestic or not foreign:
        return False
    ...
    for path in _semi_state_known_paths():
        ...
        for _, pos in (state.get("positions") or {}).items():
            if not isinstance(pos, dict):
                continue
            if str(pos.get("status", "")).upper() not in OPEN_ENTRY_STATUSES:
                continue
            if str(pos.get("user_id") or "") != tg_id:
                continue

            pos_coin = normalize_symbol(pos.get("coin") or "")
            pos_domestic = str(pos.get("domestic") or pos.get("domestic_exchange") or "").upper().strip()
            pos_foreign = str(pos.get("foreign") or pos.get("foreign_exchange") or "").upper().strip()

            if pos_coin == coin and pos_domestic == domestic and pos_foreign == foreign:
                print(f"[ROUTE LOCK] {coin} {domestic}->{foreign} ACTIVE route 보유중 / user={tg_id}")
                return True
    return False
...
def realtime_entry_recheck(signal: Dict[str, Any], amount_krw: int) -> Tuple[bool, str]:
    ...
    if REAL_ORDER_ENABLED and allowed_slippage < MIN_ALLOWED_SLIPPAGE_FOR_REAL_ENTRY_PERCENT:
        return False, (
            "❌ 진입 취소\n\n"
            "사유: 허용 슬리피지 부족\n\n"
            ...
            f"실전 최소 허용: {MIN_ALLOWED_SLIPPAGE_FOR_REAL_ENTRY_PERCENT:.2f}%\n"
            "실전에서는 진입 지연/호가변동으로 순수익이 훼손될 가능성이 큽니다."
        )
...
# ============================================================
# V9.5.4 REAL CLOSE POSITION-SIZE PATCH
# - 청산은 저장된 KRW/추정수량이 아니라 해외거래소 실제 숏 보유수량 기준으로 수행한다.
# - 같은 유저+코인+국내+해외 route는 ACTIVE 동안 신규진입 LOCK으로 막고,
#   청산 시에는 해당 해외거래소 실제 포지션 수량을 전량 reduceOnly로 닫는다.
# ============================================================
def _position_symbol_matches(position: Dict[str, Any], market: str, coin: str) -> bool:
    coin = normalize_symbol(coin)
    market_s = str(market or '').upper()
    symbols = [
        str(position.get('symbol') or ''),
        str(position.get('id') or ''),
        str(position.get('info', {}).get('symbol') if isinstance(position.get('info'), dict) else ''),
        str(position.get('info', {}).get('contract') if isinstance(position.get('info'), dict) else ''),
    ]
    for s in symbols:
        su = s.upper()
        if not su:
            continue
        if su == market_s or coin in su:
            return True
    return False
...
def _position_is_short(position: Dict[str, Any]) -> bool:
    ...
    for key in ('positiontype', 'positionside', 'side', 'holdside'):
        v = str(info.get(key) or '').lower()
        if v in ('short', 'sell'):
            return True
        if v in ('long', 'buy'):
            return False
    amt = safe_float(info.get('positionAmt') or info.get('position_amt'))
    if amt < 0:
        return True
    return 'short' in text


def _position_contract_amount(position: Dict[str, Any]) -> float:
    info = position.get('info') if isinstance(position.get('info'), dict) else {}
    candidates = [
        position.get('contracts'), position.get('amount'),
        info.get('holdVol'), info.get('vol'), info.get('available'), info.get('positionAmt'),
        info.get('position_amt'), info.get('size'), info.get('openVolume'), info.get('currentQty'),
    ]
    vals = []
    for v in candidates:
        f = abs(safe_float(v))
    ...
    if not vals:
        return 0.0
    if safe_float(position.get('contracts')) > 0:
        return abs(safe_float(position.get('contracts')))
    return vals[0]


def fetch_actual_short_position_amount(foreign_ex: Any, future_market: str, coin: str) -> Tuple[float, float, str]:
    """해외거래소 실제 숏 보유수량 조회.

    return = (order_amount_for_exchange, base_coin_qty, source_detail)
    order_amount_for_exchange: create_order amount에 넣을 수량(대개 계약수/코인수)
    base_coin_qty: 국내 현물 매도 기준 수량. contractSize 반영.
    """
    _safe_load_markets(foreign_ex)
    contract_size = 1.0
    try:
        m = (getattr(foreign_ex, 'markets', {}) or {}).get(future_market) or {}
        contract_size = safe_float(m.get('contractSize'), 1.0) or 1.0
    except Exception:
        contract_size = 1.0
    ...
    positions = []
    errors = []
    for call_args in (([future_market],), tuple()):
        try:
            if not hasattr(foreign_ex, 'fetch_positions'):
                break
            got = foreign_ex.fetch_positions(*call_args)
    ...
    for p in positions:
        if not isinstance(p, dict):
            continue
        if not _position_symbol_matches(p, future_market, coin):
            continue
        amt = _position_contract_amount(p)
        if amt <= 0:
            continue
        side = str(p.get('side') or '').lower()
        if _position_is_short(p):
            best_amount = max(best_amount, amt)
            best_side = side or 'short'
        elif best_amount <= 0 and not side:
            best_amount = max(best_amount, amt)
            best_side = 'unknown'

    if best_amount > 0:
        base_qty = best_amount * contract_size
        return best_amount, base_qty, f"actual_position amount={best_amount} base_qty={base_qty} contractSize={contract_size} side={best_side}"

    err = ' / '.join(errors[-2:]) if errors else 'position not found'
    return 0.0, 0.0, f"actual_position_unavailable: {err}"


def execute_auto_close_orders(pos: Dict[str, Any], current_edge: float) -> Tuple[bool, str]:
    """
    V9.4.6 청산 패치:
    - 청산 순서를 해외 숏 청산 → 성공 응답 확인 → 국내 현물 매도로 통일한다.
    - 해외 청산 실패 시 국내 현물은 절대 매도하지 않는다.
    - FUTURES_CLOSED_ONLY 상태면 해외는 재시도하지 않고 국내 매도만 시도한다.
    - SPOT_CLOSED_ONLY 상태면 국내는 재시도하지 않고 해외 청산만 시도한다.
    - 둘 다 성공 또는 이미 처리된 경우에만 True를 반환한다.
    """
    ...
    pos_status = str(pos.get("status") or "ACTIVE").upper()
    futures_already_closed = bool(pos.get("futures_closed")) or pos_status == "FUTURES_CLOSED_ONLY"
    spot_already_closed = bool(pos.get("spot_closed")) or pos_status == "SPOT_CLOSED_ONLY"
    ...
    # 1) 해외 선물 숏 청산 먼저. 실패하면 국내 현물 매도 금지.
    if futures_already_closed:
        result["foreign_close_success"] = True
        result["foreign_order"] = "SKIP_ALREADY_FUTURES_CLOSED"
        print(f"[실거래 해외숏 청산 스킵] 이미 futures_closed 상태: {pos.get('pos_id')}")
    else:
        try:
            ...
            # V9.5.4: 저장수량/추정수량이 아니라 해외거래소 실제 숏 보유수량으로 청산한다.
            actual_future_amount, actual_base_qty, actual_pos_source = fetch_actual_short_position_amount(foreign_ex, future_market, coin)
            if actual_future_amount > 0:
                future_amount = actual_future_amount
                signal_like["_actual_future_close_amount"] = actual_future_amount
                signal_like["_actual_future_base_qty"] = actual_base_qty
                signal_like["_actual_future_position_source"] = actual_pos_source
                print(f"[실제포지션 기준 청산수량] {foreign.upper()} {coin} {actual_pos_source}")
            else:
                future_amount = _calc_future_contract_amount(signal_like, foreign_entry_krw)
                signal_like["_actual_future_position_source"] = actual_pos_source
                print(f"[실제포지션 조회 실패→추정수량 fallback] {foreign.upper()} {coin} {actual_pos_source} estimated={future_amount}")
            try:
                precise_amount = float(foreign_ex.amount_to_precision(future_market, future_amount))
                if actual_future_amount > 0 and precise_amount > actual_future_amount:
                    future_amount = actual_future_amount
                else:
                    future_amount = precise_amount
            except Exception:
                pass
            if future_amount <= 0:
            ...
            if foreign.upper() == "BINGX":
                close_params["positionSide"] = "BOTH"

            print(f"[실거래 해외숏 청산 시도/해외우선] {foreign.upper()} {future_market} buy reduceOnly amount={future_amount} params={close_params}")
            ...
            print(f"[실거래 해외숏 청산 실패/국내매도금지] {e}")
            detail = (
                "실거래 청산 미완료: ACTIVE 유지 / 해외 실패로 국내 매도 금지\n"
                f"해외숏청산 성공=False 오류={result.get('foreign_error') or '-'}\n"
                "국내현물매도 성공=False 오류=해외 청산 실패로 시도하지 않음\n"
                "상태제안: ACTIVE"
            )
            return False, detail

    # 2) 해외 청산 성공 후 국내 현물 매도.
    if spot_already_closed:
        result["domestic_close_success"] = True
        result["domestic_order"] = "SKIP_ALREADY_SPOT_CLOSED"
        print(f"[실거래 국내현물 매도 스킵] 이미 spot_closed 상태: {pos.get('pos_id')}")
    else:
        try:
            spot_market = pos.get("domestic_market") or _resolve_spot_market_for_order(domestic_ex, domestic, coin)
            # V9.5.4: 국내 매도수량도 해외 실제 숏 청산 기준수량에 맞춘다.
            # 같은 코인이 국내에 여러 route로 섞여 있어도 이번 route의 해외 실제 청산수량만큼만 매도한다.
            actual_base_qty_for_spot = safe_float(signal_like.get("_actual_future_base_qty"))
            if actual_base_qty_for_spot > 0:
                spot_amount = actual_base_qty_for_spot
                print(f"[국내매도 수량/해외실제수량 기준] {domestic.upper()} {coin} base_qty={spot_amount}")
            else:
                spot_amount = _calc_spot_base_amount_from_krw(signal_like, domestic_entry_krw)
                print(f"[국내매도 수량/추정금액 fallback] {domestic.upper()} {coin} amount={spot_amount}")
            ...
            print(f"[실거래 국내현물 매도 시도/해외성공후] {domestic.upper()} {spot_market} sell amount={spot_amount}")
            if domestic == "bithumb":
                creds = get_api_credentials_priority(member, "BITHUMB", "spot")
    ...
    # 해외는 닫혔는데 국내 매도 실패 → 다음 루프에서는 국내 매도만 재시도
    if result["foreign_close_success"] and not result["domestic_close_success"]:
        detail = (
            "실거래 청산 미완료: FUTURES_CLOSED_ONLY / 다음 루프에서 국내 현물 매도만 재시도\n"
            f"해외숏청산 성공=True 오류=-\n"
            f"국내현물매도 성공=False 오류={result.get('domestic_error') or '-'}\n"
            f"해외주문: {result.get('foreign_order')}\n"
            "상태제안: FUTURES_CLOSED_ONLY"
        )
        return False, detail

    # 국내만 닫힌 과거/예외 상태 → 다음 루프에서는 해외 청산만 재시도
    if result["domestic_close_success"] and not result["foreign_close_success"]:
        detail = (
            "실거래 청산 미완료: SPOT_CLOSED_ONLY / 다음 루프에서 해외 숏 청산만 재시도\n"
            f"해외숏청산 성공=False 오류={result.get('foreign_error') or '-'}\n"
            "국내현물매도 성공=True 오류=-\n"
            f"국내주문: {result.get('domestic_order')}\n"
            "상태제안: SPOT_CLOSED_ONLY"
        )
        return False, detail
...
🕒 {now_str()}"""
        ...
        if current_edge <= take_profit_edge:
            ...
            if ok:
                ...
                _update_position_fields(pos_id, update_fields)
                send_pos_msg(
...
def check_entry_for_auto_fast_paper(user_id: str, signal: Dict[str, Any], amount_krw: int) -> Tuple[bool, str, float]:
    ...
    try:
        ...
        # V9.5.2f: 재검사 시간은 진입 취소 사유가 아니다.
        # 현재 실제엣지/벽/금액 조건이 살아 있으면 2초를 넘어도 진입한다.
...
def execute_real_entry_orders(user_id: str, member: Dict[str, Any], signal: Dict[str, Any], domestic_entry_krw: int, foreign_entry_krw: int) -> Tuple[bool, str, Dict[str, Any]]:
    """실거래 진입: 해외 선물 숏 성공 확인 후 국내 현물 매수.

    V9.4.6 핵심 패치:
    - 진입 순서를 해외 숏 → 성공 응답 확인 → 빗썸/국내 현물 매수로 통일한다.
    - 해외 주문이 실패하면 국내 현물은 절대 매수하지 않는다.
    - 해외 성공 후 국내 매수가 실패하면 해외 숏을 reduceOnly buy로 되돌림 청산 시도한다.
    - 기존 빗썸 v2 직접주문 함수는 그대로 사용한다.
    """
    if not REAL_ORDER_ENABLED:
    ...
    try:
        ...
        print(f"[실거래 주문 시작/해외우선] future_short {future_market} amount={future_amount} / spot_buy {spot_market} amount={spot_amount}")

        # 1) 해외 숏 먼저 진입. 실패하면 국내 현물은 매수하지 않는다.
        params = {}
        ...
        print(f"[실거래 해외숏 성공/국내매수 진행] {future_order}")

        # 2) 해외 주문 성공 응답 이후 국내 현물 매수.
        if domestic == "bithumb":
        ...
        return True, "실거래 주문 성공: 해외 숏 성공 후 국내 현물 매수 완료", result

    except Exception as e:
        err = str(e)
        print(f"[실거래 주문 실패/해외우선] {err}")

        # 해외 숏만 체결되고 국내 현물 매수가 실패한 경우, 해외 숏을 즉시 되돌림 청산 시도.
        if result.get("foreign_order") and not result.get("domestic_order"):
            try:
                rollback_params: Dict[str, Any] = {"reduceOnly": True}
                if foreign.upper() == "BINGX":
                    rollback_params["positionSide"] = "BOTH"
                rollback = foreign_ex.create_order(future_market, "market", "buy", future_amount, None, rollback_params)
                result["foreign_rollback_order"] = rollback
                err += " / 해외 숏 되돌림 청산 시도 완료"
                print(f"[실거래 롤백 해외숏청산 성공] {rollback}")
            except Exception as re:
                result["foreign_rollback_error"] = str(re)
                err += f" / 해외 숏 되돌림 청산 실패: {re}"
                print(f"[실거래 롤백 해외숏청산 실패] {re}")
...
def perform_auto_entry_for_member(member: Dict[str, Any], signal: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if member_has_active_coin_position(member, signal):
        return False, f"{signal.get('coin')} {signal.get('domestic')}->{signal.get('foreign')} ACTIVE route 포지션 보유중"
    ...
    try:
        ...
        # V9.5.2f: 재검사 시간 초과만으로는 취소하지 않는다.
        # 현재 실제엣지 >= 1.5%, 벽/금액/잔액/펀딩/LOCK 조건이 통과하면 진입한다.
        if recheck_elapsed > MAX_AUTO_RECHECK_DELAY_SEC:
            print(f"[재검사 시간 경고/진입판단 유지] elapsed={recheck_elapsed:.2f}s / 기준={MAX_AUTO_RECHECK_DELAY_SEC:.2f}s")
...
※ 한쪽 주문만 체결되었을 가능성이 있으면 거래소에서 즉시 직접 확인하세요."""
            ...
            signal = enrich_signal_with_order_trace(signal)
            print(f"[ORDER TRACE 저장 준비] domestic_order_id={signal.get('domestic_order_id')} foreign_order_id={signal.get('foreign_order_id')} domestic_filled_qty={signal.get('domestic_filled_qty')} foreign_filled_qty={signal.get('foreign_filled_qty')}")
...
def auto_entry_approved_members(signal: Dict[str, Any]) -> None:
    ...
    Existing scan bots must not run user lookup, DM, final recheck or orders.
    They only enqueue the detected candidate. The order worker handles:
    - approved member lookup
    - auto settings
    - global lock
    - final recheck
    - foreign short -> success -> Bithumb buy
    - ACTIVE save
    - DM
    """
    try:
        ok = enqueue_order_candidate(signal)
        if ok:
            print(f"[AUTO QUEUE] 후보 저장 완료 / {signal.get('coin')} {signal.get('domestic')}->{signal.get('foreign')}")
        else:
            print(f"[AUTO QUEUE] 후보 저장 스킵/실패 / {signal.get('coin')} {signal.get('domestic')}->{signal.get('foreign')}")
    except Exception as e:
        print(f"[AUTO QUEUE 예외] {signal.get('coin') if isinstance(signal, dict) else '-'} / {e}")
...
def scan_once(spot_exs: Dict[str, Any], future_exs: Dict[str, Any]) -> None:
    ...
    print(f"[V9.5.2 MEXC 510 완화] workers={workers} / 대상={len(all_spots)}개 / 요청간격={MEXC_REQUEST_INTERVAL_SEC:.2f}s / 510쿨다운={MEXC_RATE_LIMIT_COOLDOWN_SEC:.0f}s / 자동진입금액={fmt_man_krw(AUTO_ENTRY_DEFAULT_KRW)}")
            ...
            for future_ex_name, fex in future_exs.items():
                try:
                    ...
                    with dispatch_lock:
                        ...
                        push_live_event_candidate(signal_row)
                        print(f"[AUTO 개인DM 스캔봇 미발송] {base} / {spot.get('source')} -> {future_ex_name} / signal_id={signal_id} / ORDER WORKER가 진입성공·실패 DM 처리")
                        alerts_local += 1
                        cut("알림")
