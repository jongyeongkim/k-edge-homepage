...
def _semi_state_path_for_exchange(exchange_name: str) -> str:
    ...
    name_map = {
        ...
        "BINGX": "semi_auto_state_mexc.json",
    }
...
def auto_entry_approved_members(signal: Dict[str, Any]) -> None:
    """AUTO 전용: 승인회원 + auto_settings를 읽어 개인DM 알림/자동진입을 분리 처리한다.

    V9.4.3c 핵심:
    - 유료방 알림이 발생한 신호는 service_enabled=true 승인회원에게 개인 DM도 무조건 발송한다.
    - auto_enabled=false여도 개인 알림은 간다.
    - auto_enabled=true일 때만 자동진입을 시도한다.
    - auto_settings의 빈 tg_chat_id가 승인회원 tg_chat_id를 덮어쓰지 않도록 raw_member의 tg_chat_id를 우선 보존한다.
    """
    ...
    if not AUTO_ENTRY_ENABLED:
        print("[AUTO] AUTO_ENTRY_ENABLED=False - 개인DM 알림만 처리")
    elif not is_auto_entry_currently_enabled():
        print("[AUTO] 전체 정지 상태 - 개인DM 알림만 처리")
    ...
    for raw_member in members:
        ...
        # 개인 DM과 자동진입 대상 식별은 반드시 승인회원 kedge_requests의 tg_chat_id를 우선 사용한다.
        ...
        # 1) 개인 DM 알림은 auto_enabled와 무관하게 먼저 발송
        mode_text = "" if global_auto_available else "현재 전체 자동진입은 정지 상태라 알림만 전송됩니다."
        if send_approved_member_signal_dm(tg_id, signal, member, mode_text):
            alert_count += 1
        else:
            alert_fail_count += 1

        # 2) 자동진입은 auto_enabled=true + 전체 자동진입 가능 상태에서만 진행
    ...
    print(
        f"[AUTO 결과] 개인DM {alert_count} / DM실패 {alert_fail_count} / "
