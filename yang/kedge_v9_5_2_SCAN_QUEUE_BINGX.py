...
MIN_ALLOWED_SLIPPAGE_FOR_REAL_ENTRY_PERCENT = float(os.getenv("MIN_ALLOWED_SLIPPAGE_FOR_REAL_ENTRY_PERCENT", "0.15"))
...
def member_has_active_coin_position(member: Dict[str, Any], signal: Dict[str, Any]) -> bool:
    """해당 회원이 같은 코인의 미종료 포지션을 이미 보유 중인지 확인.

    V9.4.6:
    - 4파일 분리 실행 중에도 같은 코인 중복 진입을 막기 위해 모든 semi_auto_state_*.json을 조회한다.
    - ACTIVE뿐 아니라 FUTURES_CLOSED_ONLY / SPOT_CLOSED_ONLY도 아직 위험 노출이 남은 포지션으로 본다.
    """
    ...
    try:
        ...
        if recheck_elapsed > MAX_AUTO_RECHECK_DELAY_SEC:
            ...
            reason = (
                ...
                f"허용: {MAX_AUTO_RECHECK_DELAY_SEC:.2f}초\n"
                "실전 양방 기준상 시간이 지나 엣지 훼손 가능성이 큽니다."
            )
...
def push_live_event_candidate(signal: Dict[str, Any]) -> None:
    ...
    try:
        ...
        event_row = {
            ...
            "domestic_exchange": signal.get("domestic") or signal.get("domestic_exchange"),
            "foreign_exchange": signal.get("foreign") or signal.get("foreign_exchange"),
            "real_edge_percent": round(safe_float(signal.get("real_edge")), 4),
            "executable_krw": int(safe_float(signal.get("executable_krw") or signal.get("krw") or signal.get("max_entry_krw") or signal.get("final_entry_krw"))),
        ...
        print("[LIVE Supabase] CANDIDATE 저장 완료")
